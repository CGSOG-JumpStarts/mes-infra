locals {
  tags            = { ProjectName = "MES Jump Start", BuildNumber = var.buildNumber }
...
resource "azurerm_resource_group" "rg" {
  name     = var.resourceGroupName != "" ? var.resourceGroupName : "mesjs-${var.environmentName}"
...
module "network" {
  ...
  vnet_name                       = "mesjs-vnet-${random_string.random.result}"
  nsg_name                        = "mesjs-nsg-${random_string.random.result}"
  ddos_name                       = "mesjs-ddos-${random_string.random.result}"
  dns_resolver_name               = "mesjs-dns-${random_string.random.result}"
...
module "privateDnsZoneAzureOpenAi" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-azure-openai-vnetlink-${random_string.random.result}"
...
module "privateDnsZoneAzureAi" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-azure-ai-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneApp" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-app-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneKeyVault" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-kv-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneStorageAccountBlob" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-storage-blob-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneStorageAccountFile" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-storage-file-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneStorageAccountTable" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-storage-table-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneStorageAccountQueue" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-storage-queue-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneSearchService" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-search-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneCosmosDb" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-cosmos-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "privateDnsZoneACR" {
  ...
  resourceGroupName  = azurerm_resource_group.rg.name
  vnetLinkName       = "mesjs-acr-vnetlink-${random_string.random.result}"
  virtual_network_id = var.is_secure_mode ? module.network[0].vnet_id : null
  tags               = local.tags
  depends_on = [ module.network[0] ]
}
...
module "logging" {
  ...
  logAnalyticsName        = var.logAnalyticsName != "" ? var.logAnalyticsName : "mesjs-la-${random_string.random.result}"
  applicationInsightsName = var.applicationInsightsName != "" ? var.applicationInsightsName : "mesjs-ai-${random_string.random.result}"
  location                = var.location
  tags                    = local.tags
  skuName                 = "PerGB2018"
  resourceGroupName       = azurerm_resource_group.rg.name
  is_secure_mode                        = var.is_secure_mode
  privateLinkScopeName                  = "mesjs-ampls-${random_string.random.result}"
...
module "storage" {
  source                          = "./core/storage"
  name                            = var.storageAccountName != "" ? var.storageAccountName : "mesjsstore${random_string.random.result}"
  location                        = var.location
  tags                            = local.tags
  accessTier                      = "Hot"
...
module "kvModule" {
  ...
  name                          = "mesjs-kv-${random_string.random.result}"
  location                      = var.location
...
module "enrichmentApp" {
  ...
  name                                      = var.enrichmentServiceName != "" ? var.enrichmentServiceName : "mesjs-enrichmentweb-${random_string.random.result}"
  plan_name                                 = var.enrichmentAppServicePlanName != "" ? var.enrichmentAppServicePlanName : "mesjs-enrichmentasp-${random_string.random.result}"
  location                                  = var.location 
  tags                                      = local.tags
  sku = {
...
module "webapp" {
  ...
  name                                = var.backendServiceName != "" ? var.backendServiceName : "mesjs-web-${random_string.random.result}"
  plan_name                           = var.appServicePlanName != "" ? var.appServicePlanName : "mesjs-asp-${random_string.random.result}"
  sku = {
  ...
  appSettings = {
    ...
    APPLICATION_TITLE                       = var.applicationtitle == "" ? "MES Jump Start, built with Azure OpenAI" : var.applicationtitle
...
module "functions" { 
  ...
  name                                  = var.functionsAppName != "" ? var.functionsAppName : "mesjs-func-${random_string.random.result}"
  location                              = var.location
  tags                                  = local.tags
  keyVaultUri                           = module.kvModule.keyVaultUri
  keyVaultName                          = module.kvModule.keyVaultName 
  plan_name                             = var.appServicePlanName != "" ? var.appServicePlanName : "mesjs-func-asp-${random_string.random.result}"
  sku                                   = {
...
module "openaiServices" {
  ...
  name                            = var.openAIServiceName != "" ? var.openAIServiceName : "mesjs-aoai-${random_string.random.result}"
  location                        = var.location
  tags                            = local.tags
  resourceGroupName               = azurerm_resource_group.rg.name
...
module "aiDocIntelligence" {
  ...
  name                          = "mesjs-docint-${random_string.random.result}"
  location                      = var.location
  tags                          = local.tags
  customSubDomainName           = "mesjs-docint-${random_string.random.result}"
  resourceGroupName             = azurerm_resource_group.rg.name
  key_vault_name                = module.kvModule.keyVaultName
  is_secure_mode                = var.is_secure_mode
...
module "cognitiveServices" {
  ...
  name                          = "mesjs-aisvc-${random_string.random.result}"
  location                      = var.location 
  tags                          = local.tags
  resourceGroupName             = azurerm_resource_group.rg.name
  is_secure_mode                = var.is_secure_mode
...
module "searchServices" {
  source                        = "./core/search"
  name                          = var.searchServicesName != "" ? var.searchServicesName : "mesjs-search-${random_string.random.result}"
  location                      = var.location
  tags                          = local.tags
...
module "cosmosdb" {
  source = "./core/db"
  name                          = "mesjs-cosmos-${random_string.random.result}"
  location                      = var.location
  tags                          = local.tags
...
module "acr"{
  ...
  name                  = "mesjsacr${random_string.random.result}" 
  location              = var.location
  resourceGroupName     = azurerm_resource_group.rg.name
  is_secure_mode        = var.is_secure_mode
...
module "azMonitor" {
  ...
  location          = var.location
  logWorkbookName   = "mesjs-lw-${random_string.random.result}"
  resourceGroupName = azurerm_resource_group.rg.name 
