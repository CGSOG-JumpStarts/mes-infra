...
resource "azuread_application" "aad_web_app" {
  ...
  display_name                  = "mesjs_web_access_${var.randomString}"
  identifier_uris               = ["api://mesjs-${var.randomString}"]
  owners                        = local.owner_ids
  sign_in_audience              = "AzureADMyOrg"
  oauth2_post_response_required = true
  service_management_reference = var.serviceManagementReference
  web {
    redirect_uris = ["https://mesjs-web-${var.randomString}.${var.azure_websites_domain}/.auth/login/aad/callback"]
    implicit_grant {
...
resource "azuread_application" "aad_mgmt_app" {
  count             = var.isInAutomation ? 0 : 1
  display_name      = "mesjs_mgmt_access_${var.randomString}"
  owners            = local.owner_ids
  sign_in_audience  = "AzureADMyOrg"
  service_management_reference = var.serviceManagementReference
}
